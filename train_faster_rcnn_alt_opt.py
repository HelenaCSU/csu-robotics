...
from genProto import GenProtoText
import argparse
import pprint
import numpy as np
import sys, os
...
def generateList(stepmin, stepmax, stepst, wdmin, wdmax, wdst):
    L = [(i, j) for i in range(stepmin, stepmax, stepst) for j in range(wdmin, wdmax, wdst)]
    return L
...
if __name__ == '__main__':
    ...
    # 循环里更改stepsize 和weight_decay的值
    gen = GenProtoText()
    L=generateList(2000,3000,1000,1,10,1)
    for n in L:
        gen.generateList(n[0],n[1])
